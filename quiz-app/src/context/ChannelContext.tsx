...
export const ChannelProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (savedChannel && (savedChannel === '7MZ' || savedChannel === 'ENYGMA' || savedChannel === 'MELANIE' || savedChannel === 'RODRIGOZIN' || savedChannel === 'MITSKI' || savedChannel === 'M4RKIM' || savedChannel === 'ANIRAP')) {
      const isGeek = savedChannel === '7MZ' || savedChannel === 'ENYGMA' || savedChannel === 'RODRIGOZIN' || savedChannel === 'M4RKIM' || savedChannel === 'ANIRAP';
  ...
  useEffect(() => {
    if (!isLoaded) return;
    document.body.classList.remove('theme-7mz', 'theme-enygma', 'theme-melanie', 'theme-rodrigozin', 'theme-mitski', 'theme-m4rkim', 'theme-anirap');
